...
describe('Paystack Resources', () => {
  const mockFetch = mock((url: any, init: any) =>
    Promise.resolve(
      new Response(
        JSON.stringify({
          status: true,
  ...
  describe('Splits', () => {
    ...
    test('addSubaccount sends correct payload', async () => {
      await client.splits.addSubaccount(123, {
        subaccount: 'SUB_123',
        share: 20,
      })
      expect(mockFetch).toHaveBeenCalledTimes(1)
  ...
  describe('Subaccounts', () => {
    ...
    test('update sends correct payload', async () => {
      await client.subaccounts.update('SUB_123', {
  ...
  describe('Virtual Accounts', () => {
    ...
    test('requery calls correct endpoint', async () => {
      await client.virtualAccounts.requery({
        account_number: '0000000000',
        provider_slug: 'bank',
      })
      expect(mockFetch).toHaveBeenCalledTimes(1)
  ...
  describe('Transfers', () => {
    ...
    test('finalize sends correct payload', async () => {
      await client.transfers.finalize({
  ...
  describe('Misc', () => {
    ...
    test('resolveAccount sends correct query params', async () => {
      await client.misc.resolveAccount({
        account_number: '0001234567',
        bank_code: '058',
      })
      expect(mockFetch).toHaveBeenCalledTimes(1)
  ...
  describe('Terminal', () => {
    ...
    test('sendEvent sends correct payload', async () => {
      await client.terminal.sendEvent('term_123', {
        type: 'invoice',
        action: 'process',
        data: { id: 'inv_123', reference: 'ref_123' },
      })
      expect(mockFetch).toHaveBeenCalledTimes(1)
      const [url, init] = mockFetch.mock.calls[0]!
  ...
  describe('Transfer Control', () => {
    test('resendOtp sends correct payload', async () => {
      await client.transferControl.resendOtp({
        transfer_code: 'TRF_123',
        reason: 'resend',
      })
      expect(mockFetch).toHaveBeenCalledTimes(1)
